...
- **两个令牌，两个范围**（契约见 `xcollector-backend/docs/api.md`「通用约定」）：
  - **网页令牌 `WEB_API_TOKEN`** —— 用户在登录页输入的就是它。后端与 bot 都认它，
    但它只能**读取、提交人工修正、标记已读**。
  - **管理令牌 `API_TOKEN` / `BOT_API_TOKEN`** —— 只在服务器上，登录页「高级」里可选填。
    填了这个浏览器就解锁「发送到 QQ」这类会真的动手的操作。
  读接口**始终**走网页令牌（即使「高级」里填了管理令牌）—— 全权令牌不该为了看个列表
  就留在浏览器里。
- **401 才登出，403 不登出**：401 是「令牌不对」（去重新登录），403 是「令牌有效但没这个
  权限」（网页令牌碰上了 bot 专属接口）。把 403 也当成登录过期，用户会被莫名其妙踢回
  登录页、重输一遍还是 403。
...
- **这不是账号体系**：它是一个**共享密钥**，不是按用户的登录。
  **所有拿同一个网页令牌登录的人权限完全一样**，没有角色、没有审计、没法单独吊销某个人
  （要换密钥只能改后端 `WEB_API_TOKEN`，所有人一起重输）。
  令牌分级只解决「泄露网页令牌会不会导致数据被改」—— 不解决「谁看了什么」。
